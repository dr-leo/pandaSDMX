...
class InseeTestCase(unittest.TestCase):
    ...
    def test_load_dataset(self):
        ...
        '''load all dataflows'''
        dataflows_response = self.sdmx.get(resource_type='dataflow', agency='FR1', fromfile=DATAFLOW_FP)
        ...
        datastructure_response = self.sdmx.get(resource_type='datastructure', agency='FR1', fromfile=fp_datastructure)
        self.assertTrue(dataset_code in datastructure_response.msg.datastructures)
        dsd = datastructure_response.msg.datastructures[dataset_code]
        
        '''Verify dimensions list'''
        dimensions = OrderedDict([dim.id, dim] for dim in dsd.dimensions.aslist() if dim.id not in ['TIME', 'TIME_PERIOD'])
        ...
        data = self.sdmx.get(resource_type='data', agency='FR1', fromfile=fp_data)
    ...
    def test_fixe_key_names(self):
        ...
        dimensions = OrderedDict([dim.id, dim] for dim in dsd.dimensions.aslist() if dim.id not in ['TIME', 'TIME_PERIOD'])
        dim_keys = list(dimensions.keys())
        self.assertEqual(dim_keys, ['SECT-INST', 'OPERATION', 'PRODUIT', 'PRIX'])
