...
class Writer(BaseWriter):
    ...
    #     # Preparations
    #     dim_at_obs = self.msg.observation_dimension
    #
    ...
    #     elif (hasattr(source, 'data') and dim_at_obs != AllDimensions and
    #           len(source.data) == 1):
    #         iter_series = source.data.series[0]
    #
    #     # Is 'data' a flat dataset with just a list of obs?
    #     if dim_at_obs == AllDimensions:
    #         idx = generate_index(source.data.obs)
    #
    #         if dtype:
    #             values_series = pd.Series(
    #                 [obs.value for obs in source.data.obs],
    #                 dtype=dtype, index=idx)
    #         if attributes:
    #             obs_attrib = np.asarray(
    #                 [obs.attrib for obs in source.data.obs],
    #                 dtype='object')
    ...
    #         obs_zip = series.iter_obs(dtype, attributes, reverse_obs)
