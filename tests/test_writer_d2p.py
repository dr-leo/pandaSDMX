...
def test_write_arguments(empty_req):
    msg = empty_req.get(fromfile=test_data[0]).msg
...
def test_write_json(empty_req, path):
...
@pytest.mark.parametrize('path', test_data)
def test_write_json_attributes(empty_req, path):
    msg = empty_req.get(fromfile=path).msg
